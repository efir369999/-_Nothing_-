"""
Ɉ Montana Heartbeat Structures v3.7

Layer 1: Participation Heartbeats per MONTANA_TECHNICAL_SPECIFICATION.md §7.

Heartbeats are the core participation mechanism:
- FullHeartbeat: From Full Nodes, includes VDF proof (~3.5KB with ML-DSA signature)
- LightHeartbeat: From Light Nodes, simpler structure (~3.4KB with ML-DSA signature)
...
class FullHeartbeat:
    ...
    Size: ~3,500 bytes (with ML-DSA signature)
    """
    ...
    # Signature
    signature: Signature              # ML-DSA signature (3,310 bytes)
    ...
    def deserialize(cls, data: bytes) -> Tuple["FullHeartbeat", int]:
        ...
        r.offset += 1 + ML_DSA_SIGNATURE_SIZE
...
class LightHeartbeat:
    ...
    Size: ~3,400 bytes (with ML-DSA signature)
    """
    # Header (fixed size: 109 bytes)
    version: int                      # Protocol version (1 byte)
...
def verify_heartbeat_signature(heartbeat: Heartbeat) -> bool:
    ...
    return mldsa_verify(heartbeat.public_key, message, heartbeat.signature)
...
def sign_heartbeat(heartbeat: Heartbeat, secret_key) -> Heartbeat:
    ...
    message = heartbeat.serialize_for_signing()
    signature = mldsa_sign(secret_key, message)

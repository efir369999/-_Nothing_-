...
\begin{tabular}{|l|l|}
\hline
\textbf{Blockchain} & \textbf{Timechain} \\
\hline
Chain of blocks & Chain of time \\
Secured by work or stake & Secured by physics \\
``Who computed first is right'' & ``Time passed---this is fact'' \\
Unit: block & Unit: second \\
\hline
\end{tabular}
\end{center}
...
\section{Sequential Hash Chain}

Montana uses a sequential hash chain as proof of participation in a time window---not computation speed:
...
The construction uses iterated hashing:

$$H^T(\text{input}) = \text{SHAKE256}(\text{SHAKE256}(...\text{SHAKE256}(\text{input})...))$$

where $T = 2^{24}$ iterations.  Verification uses STARK proofs for $O(\log T)$ validation without recomputing $T$ iterations.

\textbf{Terminology note.}  This is not a classical VDF in the Boneh et al. sense.  Classical VDFs use algebraic structures (RSA groups, class groups) with mathematical sequentiality guarantees.  Montana's construction is a sequential hash chain with empirical (Type C) security: no shortcut for iterated SHAKE256 is known, but one is theoretically possible if internal structure is discovered.

Montana accepts this tradeoff for post-quantum security.  SHAKE256 has no known quantum speedup beyond Grover's $\sqrt{T}$, while RSA-based VDFs are broken by Shor's algorithm.
...
We have proposed a Timechain---a system where time itself is the chain, secured by physics rather than economics.  The fundamental unit is not a block, but a second.
...
Montana requires minimal assumptions: that nodes maintain reasonable clock accuracy (within $\pm$5 seconds), and that the hash function admits no iteration shortcut.  Both are empirically verified properties of existing systems.
